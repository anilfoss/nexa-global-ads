...
const CareerModal = ({ isOpen, onClose, onOpenChange, job }) => {
    ...
    useGSAP(() => {
        const ctx = gsap.context(() => {
            ...
            const locations = gsap.utils.toArray(
                ".job-modal .locations-wrapper > *"
            );
            const chips = gsap.utils.toArray(".job-modal .chip-wrapper > *");
            const content = gsap.utils.toArray(
                ".job-modal .content-wrapper > *"
            );
            const apply = document.querySelector(".job-modal .apply-wrapper");
            const form = document.querySelector(".job-modal form");

            const tl = gsap.timeline({});

            tl.to(".job-modal .desc-animation", {
                opacity: 1,
            })
                .to(
                ...
                {(onClose) => (
                    <>
                        <ModalBody className="p-6 md:p-10">
                            <h3 className="desc-animation">{job.position}</h3>
                                    ...
                                    {job?.locations?.map((location, index) => {
                                        return (
                                            <Chip
                                                ...
                                                className="chip bg-green-dark bg-opacity-10 px-1.5 py-1.5 h-auto opacity-0 relative top-4"
                                            >
                                                {location}
                                            </Chip>
                                        );
                            ...
                            {job?.tags.length > 0 && (
                                <div className="chip-wrapper flex flex-wrap gap-3">
                                    {job?.tags.map((tag, index) => (
                                        <Chip
                                            key={index}
                                            variant="bordered"
                                            className="px-2 py-0.5 border-1 border-black h-auto opacity-0 relative top-4"
                                            classNames={{
                                                content:
                            ...
                            <div className="flex flex-col gap-3 mt-6">
                                {job?.qualifications.length > 0 && (
                                    <div className="content-wrapper mt-3">
                                        <h6 className="font-medium mb-2 opacity-0 relative top-8">
                                            Qualifications{" "}
                                            <span className="font-body">:</span>
                                        </h6>
                                        <ul className="list-disc pl-6 -mb-1 opacity-0 relative top-8">
                            ...
                            <div className="apply-wrapper flex items-center my-6 opacity-0 relative top-4">
